...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate_username(self, value):
        if User.objects.filter(username=value).exists():
            raise serializers.ValidationError('User is already exists')
        return value

    def validate_email(self, value):
        if User.objects.filter(email=value).exists():
            raise serializers.ValidationError('User this email is already exists')
        return value
    ...
    def validate(self, attrs):
        ...
        if username and password:
            user = authenticate(
                ...
                password=password)
            if not user:
                raise serializers.ValidationError('Authentification is error',
                code='authorization')
        else:
